    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (title != null && description != null && videoUrl != null) {
          ...
          print('Required metadata missing in the live stream response.');
        }
      } else {
        print('Failed to fetch live stream. HTTP Status: ${response.statusCode}');
      }
      ...
      if (response.statusCode == 200) {
        ...
        for (var element in streamElements) {
          ...
          if (titleElement != null && dateElement != null && urlElement != null) {
            ...
            if (!streams.any((s) =>
                s['title']?.toLowerCase() == stream['title']?.toLowerCase() && s['url'] == stream['url'])) {
              streams.add(stream);
        ...
        print('Failed to fetch archive streams. HTTP Status: ${response.statusCode}');
      }
    } on http.ClientException catch (e) {
      print('Client exception: $e');
      throw Exception(ErrorHelper.getErrorMessage(e));
